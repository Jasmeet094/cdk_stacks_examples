...
class Stack(stack):
    ...
    def __init__(
        ...
        # Import and Find Existing Security Group for ECS Service test-ng-jpm-mock-api # noqa
        ...
        Tags.of(self.load_balancer.alb).add(
            "Applications",
            f"test-ng-jpm-mock-api-{environment}"
        )
        ...
        # ECS Services
        self.test_ng_jpm_mock_api = self.create_test_ng_jpm_mock_api(
    ...
    def create_test_ng_jpm_mock_api(
        self,
        metadata,
        environment,
        execution_role
        ...
        secret = sm.Secret.from_secret_name_v2(
            self,
            "test-ng-jpm-mock-api-secret",
            metadata.get("test_ng_jpm_mock_api")["SECRET_NAME"])
        for key in EnvVariables.test_ng_jpm_mock_api:
        ...
        return ECSService(
            ...
            priority=30,
            container_port=443,
            memory_limit_mib=metadata.get("test_ng_jpm_mock_api")[
                "memory_limit_mib"],
            cpu=metadata.get("test_ng_jpm_mock_api")["cpu"],
            env_vars=env_vars,
