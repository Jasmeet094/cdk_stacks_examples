...
class Stack(stack):
    def __init__(
        ...
        # Import and Find Existing Security Group for ECS Service test-ng-portal-api # noqa
        ...
        Tags.of(self.load_balancer.alb).add(
            "Applications",
            f"test-ng-app-{environment} test-ng-portal-api-{environment}"
        )
        ...
        # test portal service
        self.test_ng_portal = self.create_test_app(
            metadata, environment, ecs_exe_role)

        self.test_ng_portal_api = self.create_test_ng_portal_api(
            metadata, environment, ecs_exe_role)

    def get_route53_function(self, construct_id, metadata):
        return lambda_.Function(
            self,
            "lambda-fargate-" + construct_id,
    ...
    def create_test_app(self, metadata, environment, execution_role):
        secret_vars = {}
        env_vars = {}
        # test-app environment variables injected by CircleCI context

        # latest image version id
        commit_id = ssm.StringParameter.value_for_string_parameter(
            self, "test-ng-app-commit-id")

        return ECSService(
            self,
            "test-ng-app",
            metadata.get('test_app')["image"] + ":" + commit_id,
            ...
            priority=20,
            container_port=443,
            memory_limit_mib=metadata.get("test_app")["memory_limit_mib"],
            cpu=metadata.get("test_app")["cpu"],
            env_vars=env_vars,
    ...
    def create_test_ng_portal_api(
        ...
        secret = sm.Secret.from_secret_name_v2(
            self,
            "test-ng-portal-api-secret",
            metadata.get("test_ng_portal_api")["SECRET_NAME"])
        for key in EnvVariables.test_ng_portal_api:
        ...
        # latest image version id
        commit_id = ssm.StringParameter.value_for_string_parameter(
            self, "test-ng-portal-api-commit-id")

        # ECS Service for test-nextgen-api
        return ECSService(
            self,
            "test-ng-portal-api",
            metadata.get("test_ng_portal_api")["image"] + ":" + commit_id,
            cluster=self.cluster,
            vpc=self.vpc,
            load_balancer=self.load_balancer,
            ...
            priority=30,
            container_port=443,
            memory_limit_mib=metadata.get("test_ng_portal_api")[
                "memory_limit_mib"],
            cpu=metadata.get("test_ng_portal_api")["cpu"],
            env_vars=env_vars,
            task_role=execution_role,
            execution_role=execution_role,
            metadata=metadata,
