...
class Stack(stack):
    ...
    def __init__(
        ...
        Tags.of(self.load_balancer.alb).add(
            "Applications",
            f"test-ng-services-api-{environment}")
        ...
        # ECS Services
        self.test_ng_services_api = self.create_test_ng_services_api(
    ...
    def create_test_ng_services_api(
        self,
        metadata,
        environment,
        execution_role
        ...
        secret = sm.Secret.from_secret_name_v2(
            self,
            "test-ng-services-api-secret",
            metadata.get("test_ng_services_api")["SECRET_NAME"])
        for key in EnvVariables.test_ng_services_api:
        ...
        commit_id = ssm.StringParameter.value_for_string_parameter(
            self, "test-ng-services-api-commit-id")

        # ECS Service for test-nextgen-api
        return ECSService(
            self,
            "test-ng-services-api",
            metadata.get("test_ng_services_api")["image"] + ":" + commit_id,
            ...
            priority=20,
            container_port=443,
            memory_limit_mib=metadata.get("test_ng_services_api")[
                "memory_limit_mib"],
            cpu=metadata.get("test_ng_services_api")["cpu"],
            env_vars=env_vars,
